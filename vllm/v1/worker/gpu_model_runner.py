        ...
        for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
            ...
            if builder is not None:
                buf = builder.reserve(
                    ...
                    # stride spans the whole per-block window), so they commit
                    # as one contiguous run of blocks whatever the declared
                    # layout.
                    1
                    if kv_cache_tensor.block_stride is not None
                    else tensor_num_outer_segments(
                        kv_cache_tensor, layer_specs, layout
                    ),
                )
            else:
                buf = torch.zeros(
